...
window.openAddStudentModal = function () {
    ...
    const classList = CLASSES[session.schoolType || 'Primary'] || CLASSES['Primary'];
    document.getElementById('asClass').innerHTML = '<option value="">-- No Class Yet --</option>' +
        classList.map(c => `<option value="${c}">${c}</option>`).join('');

    // Populate teacher dropdown
    document.getElementById('asTeacher').innerHTML = '<option value="">-- No Teacher Yet --</option>' +
        allTeachersCache.map(t => `<option value="${t.id}">${t.name}</option>`).join('');
...
document.getElementById('saveAddStudentBtn')?.addEventListener('click', async () => {
    ...
    const className  = document.getElementById('asClass').value;
    const teacherId  = document.getElementById('asTeacher').value;
...
// ── 8. RENDER ADMIN GRADES (ACADEMIC TAB) ─────────────────────────────────
...
window.executeStudentPrint = function () {
    ...
    const html = `
    <!DOCTYPE html><html><head><title>Official Term Report — ${escapeHtml(student.name)}</title>
    <style>
        ...
        <div class="header-flex">
            ...
            <div class="header-text"><h1>${escapeHtml(schoolName)}</h1><h2>OFFICIAL TERM REPORT</h2></div>
        </div>
        ...
        <div class="analytics-grid">
            <div class="analytic-card"><div class="analytic-val">${cumulativeAvg}%</div><div class="analytic-lbl">Term Average</div></div>
            <div class="analytic-card"><div class="analytic-val">${gpaLetter}</div><div class="analytic-lbl">Overall Grade</div></div>
            <div class="analytic-card"><div class="analytic-val">${totalAssessments}</div><div class="analytic-lbl">Total Assessments</div></div>
        </div>
        <table>
            <thead><tr><th>Subject / Assignment</th><th class="center">Assessments / Score</th><th class="center">Average / Pct</th><th class="center">Grade</th></tr></thead>
            <tbody>${gradesHtml}</tbody>
        </table>
        <div class="footer">
            <span><strong>NOTICE:</strong> This document is an official academic report generated for <strong>${escapeHtml(schoolName)}</strong>.</span><br>
