...
// it only opens modals, drills into a subject/type, and prints. There is
...
// gradesLoader, noCurrentGradesMsg, gradesTable, subjectRows, typeModal)
...
async function loadGrades(studentId, schoolId) {
    try {
        ...
        pageStudentName  = studentData.name || 'Student';
        pageStudentClass = studentData.className || '—';

        const schoolData = schoolSnap.data() || {};
        const semId      = schoolData.activeSemesterId;

        setText('displaySchoolName', schoolData.schoolName || 'ConnectUs');
        ...
        setText('activeSemesterDisplay', semName);
        setText('gbTerm', semName);
        ...
        if (tSnap && tSnap.exists()) {
            ...
            teacherRubric    = await resolveGradeWeights(schoolId, tId, { legacyTeacherData: td }) || [];
...
// ── OPEN SUBJECT MODAL — TYPE TILES VIEW ───────────────────────────────
...
export async function initGradesPage({ studentId, schoolId }) {
