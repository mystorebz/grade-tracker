// ── SHARED REPORT BUILDER RENDERER (student + parent) ─────────────────────
// ARCHITECTURAL MANDATE: Dashboard Analytics & Parent Portal "Mirror" Sync,
// Phase 1. Extracted verbatim from student/reports/reports.js — this IS the
// "report builder" the mandate asks the Parent Portal to reuse exactly,
// filters and all. student/reports/reports.js and parent/reports/reports.js
// are now both thin wrappers: auth + layout + child resolution, then a
// single call to initReportsPage() below.
//
// READ-ONLY NOTE: "Build Report" only filters data already held in memory,
// and "Print Custom Report" only opens window.print() on a generated
// window — neither ever writes to Firestore. This page was already
// strictly read-only viewing-and-exporting before this mandate; nothing
// needs to be stripped for the parent read-only requirement.
//
// Unlike render-grades.js/render-history.js, this module never had a
// `session.studentData` shortcut to begin with — student/reports/reports.js
// already read session.studentData directly inline. Since a parent session
// carries no such per-child field, this module resolves the student doc
// itself as its very first step, then proceeds exactly as the original did.
...
import { letterGrade, gradeColorClass, calculateWeightedAverage, resolveGradeWeights } from './utils.js';
...
// ── INITIALIZE DATA — Fix 6: fully parallelized ────────────────────────
// Phase 0: the student's own doc (name/className/teacherId) — the one
// lookup this module needs that the original didn't, since a parent's
// session carries no equivalent of session.studentData for the child.
...
async function initializeReports() {
    try {
        const studentSnap = await getDoc(doc(db, 'students', pageStudentId));
        const studentData = studentSnap.exists() ? studentSnap.data() : {};
        studentDisplayName  = studentData.name || 'Student';
        studentDisplayClass = studentData.className || 'Unassigned Class';
        const tId = studentData.teacherId;

        const [schoolSnap, tSnap, semSnap, gSnap] = await Promise.all([
            getDoc(doc(db, 'schools', pageSchoolId)),
            tId ? getDoc(doc(db, 'teachers', tId)) : Promise.resolve(null),
            getDocs(collection(db, 'schools', pageSchoolId, 'semesters')),
            getDocs(query(
        ...
        if (schoolSnap.exists()) {
            ...
            setText('displaySchoolName', schoolData.schoolName || 'ConnectUs School');
        }
        setText('displayStudentName', studentDisplayName);
        setText('displayStudentClass', studentDisplayClass);
        ...
        setText('activeSemesterDisplay', activeSemObj ? activeSemObj.name : 'Unknown');
            ...
            uniqueTeacherIds.map(async id => {
                if (!teacherRubricsCache[id]) {
                    try {
                        ...
                        teacherRubricsCache[id] = await resolveGradeWeights(pageSchoolId, id, { legacyTeacherData: legacyData }) || [];
                    } catch (e) {
...
// ── PRINT GENERATOR — Fix 4 (no banner) + Fix 5 (consistent style) ────
function printDocument() {
    ...
    const studentClass  = studentDisplayClass;
...
// ── PUBLIC ENTRY POINT ────────────────────────────────────────────────
export async function initReportsPage({ studentId, schoolId }) {
