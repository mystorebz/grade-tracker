// ── SHARED ACADEMIC HISTORY RENDERER (student + parent) ───────────────────
// ARCHITECTURAL MANDATE: Dashboard Analytics & Parent Portal "Mirror" Sync,
// Phase 1. Extracted verbatim from student/history/history.js so the Parent
// Portal's new Academic History page renders from the EXACT same code the
// student sees. student/history/history.js and parent/history/history.js
// are now both thin wrappers: auth + layout + child resolution, then a
// single call to initHistoryPage() below.
//
// READ-ONLY NOTE: like Grades, this page has zero write actions in its
// student form — accordions, a details modal, and a print action. Nothing
// needs to be stripped for the parent read-only mandate.
//
// DOM ids below (historySemesterSelect, historySubjectsContainer,
// noHistoryGradesMsg, historyInfoCard, historyTeacherName, the
// assignmentModal/-Inner and its fields) are page-body markup, identical in
// both student/history/history.html and parent/history/history.html by
// construction. displaySchoolName and activeSemesterDisplay are
// layout-owned ids guarded the same way render-grades.js guards them.
import { db } from './firebase-init.js';
import { collection, query, where, getDocs, doc, getDoc } from "https://www.gstatic.com/firebasejs/10.7.1/firebase-firestore.js";
import { calculateWeightedAverage, resolveGradeWeights } from './utils.js';

// ── ELEMENTS (module load happens after DOM parse — script is at end of body) ──
...
// ── ACADEMIC PASSPORT — school transfer timeline ───────────────────────
...
async function initializeHistory() {
    try {
        const schoolSnap = await getDoc(doc(db, 'schools', pageSchoolId));
        if (schoolSnap.exists()) {
            setText('displaySchoolName', schoolSnap.data().schoolName);
            schoolActiveSemesterId = schoolSnap.data().activeSemesterId;
        }

        const tSnap = await getDocs(query(collection(db, 'teachers'), where('currentSchoolId', '==', pageSchoolId)));
        tSnap.forEach(d => { teachersMap[d.id] = d.data().name; });

        const semSnap = await getDocs(collection(db, 'schools', pageSchoolId, 'semesters'));
        ...
        try {
            const globalStudentSnap = await getDoc(doc(db, 'students', pageStudentId));
...
async function loadHistoricalGrades() {
    ...
    try {
        ...
        const q = query(
            collection(db, 'students', pageStudentId, 'grades'),
            where('schoolId', '==', pageSchoolId)
        );
        ...
        try {
            const eSnap = await getDocs(query(
                collection(db, 'students', pageStudentId, 'evaluations'),
                where('schoolId', '==', pageSchoolId)
            ));
        ...
        for (const tId of uniqueTeacherIds) {
            if (!teacherRubricsCache[tId]) {
                try {
                    ...
                    teacherRubricsCache[tId] = await resolveGradeWeights(pageSchoolId, tId, { legacyTeacherData: legacyData }) || [];
                } catch (e) {
...
function renderUnclassifiedEvaluations(semNameFn) {
    const orphans = (currentViewEvals || []).filter(ev => ev.schoolId === pageSchoolId && !ev.className);
...
// ── PRINT PROFESSIONAL TRANSCRIPT ──────────────────────────────────────
window.printStudentRecord = async (studentId) => {
    const targetId = studentId || pageStudentId;

    // Fetch the student's own doc fresh (name/dob/className/studentIdNum) —
    // this module never assumes a `session.studentData` shape, since a
    // parent's session carries no such field for the child being viewed.
    const studentSnap = await getDoc(doc(db, 'students', targetId));
    const s = studentSnap.exists() ? studentSnap.data() : {};

    // Fetch grades specifically for this school (unlocked records)
    let gradesSnap = await getDocs(query(
        collection(db, 'students', targetId, 'grades'),
        where('schoolId', '==', pageSchoolId)
    ));
    ...
    for (const tId of printTeacherIds) {
        try {
            const tSnap = await getDoc(doc(db, 'teachers', tId));
            const legacyData = tSnap.exists() ? tSnap.data() : null;
            printRubricsCache[tId] = await resolveGradeWeights(pageSchoolId, tId, { legacyTeacherData: legacyData }) || [];
        } catch (e) {
    ...
    <html>
    <head>
        <title>Academic Transcript — ${s.name || 'Student'}</title>
        <style>
    ...
    <body>
        ...
        <div class="student-info-box">
            <div class="info-col">
                <div class="info-item"><span class="info-label">Student Name</span><span class="info-value">${s.name || 'Student'}</span></div>
                <div class="info-item"><span class="info-label">Global ID Number</span><span class="info-value" style="font-family:monospace;letter-spacing:1px;">${s.studentIdNum || targetId}</span></div>
            </div>
            <div class="info-col">
...
// ── PUBLIC ENTRY POINT ────────────────────────────────────────────────
export async function initHistoryPage({ studentId, schoolId }) {
