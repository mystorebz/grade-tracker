...
// Searchable filter values (set by the SF component below)
let sfStudentValue = '';   // student ID
let sfSubjectValue = '';   // subject name
let sfTypeValue    = '';   // grade type
...
/**
 * Wires up a searchable dropdown for a filter group.
 * @param {string} key         - 'student' | 'subject' | 'type'
 * @param {Array}  items       - [{id, label}]
 * @param {Function} onSelect  - called with selected id ('' = all)
 */
function buildSearchableFilter(key, items, onSelect) {
    ...
    function renderDropdown(term) {
        ...
        if (filtered.length === 0) {
            html += `<div class="sf-no-results">No results for "${term}"</div>`;
        } else {
            html += filtered.map(i =>
                `<div class="sf-item${i.id === selectedId ? ' sf-selected' : ''}" data-id="${escHtml(i.id)}">${escHtml(i.label)}</div>`
            ).join('');
        }
        dropdown.innerHTML = html;
        dropdown.classList.add('sf-open');
    }
    ...
    // Expose clear function via window for the HTML onclick="clearSF(...)"
    return {
        clear: () => select('', ''),
        setItems: (newItems) => {
            items = newItems;
            // Re-validate selected item still exists
...
async function init() {
    ...
    // ── Build searchable filters ───────────────────────────────────────────────
    sfType = buildSearchableFilter(
        'type',
        getGradeTypes().map(t => ({ id: t, label: t })),
...
async function loadSemestersAndLockStatus() {
    try {
        ...
        if (cached) {
            try { rawSems = JSON.parse(cached); } catch (_) { rawSems = []; }
        }
...
async function loadStudents() {
    try {
        ...
        // ── Populate student searchable filter ────────────────────────────────
        if (sfStudent) {
            sfStudent.setItems(
                [...allStudentsCache]
...
async function getAllGrades(semId) {
    ...
    await Promise.all(allStudentsCache.map(async s => {
        ...
        } catch (e) { /* per-student silent fail */ }
    }));
...
async function loadGradebook() {
    ...
    tbody.innerHTML = `<tr><td colspan="8"><div class="gb-empty">
        <i class="fa-solid fa-spinner fa-spin" style="color:#0ea871;"></i>
...
function renderGradebook() {
    ...
    allPcts.forEach(p => {
        if (p >= 90) dist.a++; else if (p >= 80) dist.b++; else if (p >= 70) dist.c++; else if (p >= 65) dist.d++; else dist.f++;
    });
    ...
    if (!rows.length) {
        tbody.innerHTML = `<tr><td colspan="8"><div class="gb-empty">
            ...
            <p>${allRows.length ? 'No grades match the selected filters.' : 'No grades logged yet for this period.'}</p>
        </div></td></tr>`;
        return;
    }
    ...
    tbody.innerHTML = rows
        ...
        .map((g, idx) => {
            ...
            return `<tr class="gb-row">
                ...
                <td>
                    <p class="gb-title">${escHtml(g.title||'—')}</p>
                    <p class="gb-date">${g.date||''}</p>
                </td>
                ...
                <td class="tc">
                    <div class="gb-grade-wrap">
                        ...
                        <div class="gb-bar-bg"><div class="gb-bar-fill" style="width:${Math.min(pct||0,100)}%;background:${barColor};"></div></div>
                    </div>
                </td>
                <td class="tr">
                    <div style="display:flex;align-items:center;justify-content:flex-end;gap:5px;">
                        <button onclick="openAssignmentModal('${g.id}')" class="gb-row-btn gb-btn-view" title="View"><i class="fa-solid fa-eye"></i></button>
                        ${actionBtns}
                    </div>
                </td>
            </tr>`;
...
// ── 10. GRADE COLOR HELPER ────────────────────────────────────────────────────
function gradeColor(pct) {
    if (pct >= 90) return '#065f46'; if (pct >= 80) return '#1e3a8a';
    if (pct >= 70) return '#134e4a'; if (pct >= 65) return '#78350f';
    return '#7f1d1d';
}
...
window.openAssignmentModal = function(gradeId) {
    ...
    if (g.historyLogs?.length) {
        histHTML = `<div style="background:#fffbeb;border:1px solid #fde68a;border-radius:4px;padding:14px;margin-top:14px;">
            ...
            <div style="display:flex;flex-direction:column;gap:6px;max-height:120px;overflow-y:auto;">
                ${g.historyLogs.map(l => `<div style="font-size:11px;color:#78350f;background:#fff;border:1px solid #fde68a;border-radius:3px;padding:7px 10px;">
                    ${typeof l==='object' ? `[${escHtml(l.changedAt)}] ${l.oldScore}/${l.oldMax} → ${l.newScore}/${l.newMax}. ${escHtml(l.reason||'')}` : escHtml(l)}
                </div>`).join('')}
            </div>
        </div>`;
    }
    document.getElementById('aModalBody').innerHTML = `
        <div style="text-align:center;margin-bottom:18px;">
            ...
            ${pct!==null ? `<div style="display:flex;align-items:center;justify-content:center;gap:10px;margin-top:8px;">
                <span class="${color}" style="font-size:16px;font-weight:700;font-family:'DM Mono',monospace;">${pct}%</span>
                <span class="${color}" style="font-size:13px;font-weight:700;padding:4px 12px;border-radius:3px;border:1px solid;${pct>=90?'background:#dcfce7;border-color:#bbf7d0;':pct>=80?'background:#dbeafe;border-color:#bfdbfe;':pct>=70?'background:#ccfbf1;border-color:#99f6e4;':pct>=65?'background:#fef3c7;border-color:#fde68a;':'background:#fee2e2;border-color:#fecaca;'}">${letterGrade(pct)}</span>
            </div>
            <div style="margin:10px 16px 0;height:6px;background:#f0f4f8;border-radius:2px;overflow:hidden;">
                <div style="height:100%;width:${pct||0}%;background:${fill};"></div>
            </div>` : ''}
        </div>
        <div style="display:flex;flex-direction:column;gap:0;margin-bottom:14px;border:1px solid #e8edf2;border-radius:4px;overflow:hidden;">
            ...
            <div style="display:flex;align-items:center;justify-content:space-between;padding:9px 14px;${i<2?'border-bottom:1px solid #f0f4f8;':''}background:#fff;">
...
async function saveEditedGrade() {
    ...
    try {
        ...
        if (changed) {
            const reason = document.getElementById('ed-reason').value.trim();
            updates.historyLogs = [...(currentEditData.historyLogs||[]),
